describe('Admin - Dashboard', () => {
    ...
    it('Creates a new staff account', () => {
      ...
      cy.get('[data-cy=staff-role]').click()
      cy.get('[data-cy=role-receptionist]').click()

      cy.get('[data-cy=staff-password]').type('securePass123')
      cy.get('[data-cy=staff-dayoff]').click()
      cy.get('div[role="listbox"]').contains('FRIDAY').click()
    ...
    it('Edits a staff account', () => {
      cy.get('[data-cy=staff-card]').first().within(() => {
        cy.get('button').click() // or add data-cy="staff-options-trigger" and use that
      })
      ...
      cy.get('[data-cy=edit-staff-role]').click()
      cy.get('div[role="listbox"]').contains('AESTHETICIAN').click()
      cy.get('[data-cy=edit-staff-dayoff]').click()
      cy.get('div[role="listbox"]').contains('MONDAY').click()
